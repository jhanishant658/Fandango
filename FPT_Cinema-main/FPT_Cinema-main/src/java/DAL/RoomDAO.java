...
public class RoomDAO {
    ...
    public Room getRoomById(int id) {
        Room r = null;
        String sql = "SELECT * FROM room_he194197 WHERE id = ?";
        try {
    ...
    public void loadRoom() {
        listRoom = new Vector<>();
        String sql = "SELECT * FROM room_he194197";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public boolean insertRoom(String name, int capacity) {
        String sql = "INSERT INTO room_he194197 (name, capacity) VALUES (?, ?)";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, name);
    ...
    public boolean updateRoom(int id, String name, int capacity) {
        String sql = "UPDATE room_he194197 SET name = ?, capacity = ? WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, name);
            ps.setInt(2, capacity);
    ...
    public boolean deleteRoomById(int id) {
        String sql = "DELETE FROM room_he194197 WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
