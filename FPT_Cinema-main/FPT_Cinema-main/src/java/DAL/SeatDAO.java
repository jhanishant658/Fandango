...
public class SeatDAO {
    ...
    public List<Seat> loadSeat(int film_id, String startHour) {
        ...
        String sql = "SELECT DISTINCT s.id, s.seat_number, s.type, s.price, s.roomid "
                + "FROM film_he194197 AS f "
                + "INNER JOIN ticket_film_he194197 AS tf ON f.id = tf.filmid "
                + "INNER JOIN room_he194197 AS r ON tf.roomid = r.id "
                + "INNER JOIN seat_he194197 AS s ON r.id = s.roomid "
