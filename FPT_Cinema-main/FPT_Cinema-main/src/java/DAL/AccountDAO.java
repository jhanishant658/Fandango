...
public class AccountDAO {
    ...
    public void loadAccount() {
        accList = new Vector<>();
        String sql = "select * from account_he194197";
        try {
    ...
    public void insertAccount(String username, String password, String firstname, String lastname, String email, String phonenumber) {
        String sql = "Insert into Account_he194197 values(?,?,?,?,?,?,?)";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public Account getAccByUsername(String username) {
        ...
        String sql = "SELECT * FROM Account_he194197 WHERE username = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, username);
            ResultSet rs = ps.executeQuery();
    ...
    public void updateAccount(int id, String first_name, String last_name, String phone_number, String email, String password) {
        String sql = "UPDATE Account_he194197 SET first_name = ?, last_name = ?, phone_number = ?, email = ?, password = ? WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public void adminUpdateAccount(int id, String username, String password, String first_name, String last_name, String email, String phone_number, String role_id) {
        String sql = "UPDATE Account_he194197 SET username = ?, password = ?, first_name = ?, last_name = ?, email = ?, phone_number = ?, role_id = ? WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, username);
            ps.setString(2, password);
    ...
    public Account getAccountById(int id) {
        Account account = null;
        String sql = "SELECT * FROM Account_he194197 WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
    ...
    public void deleteAccountById(int id) {
        String sql = "DELETE FROM Account_he194197 WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
            ps.executeUpdate();
    ...
    public void updatePasswordByUsernameAndPhoneNumber(String username, String phoneNumber, String newPassword) {
        String sql = "UPDATE Account_he194197 SET password = ? WHERE username = ? AND phone_number = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
...
//    public List<Account> getAccountsSortedBySpending(boolean ascending) {
//        List<Account> list = new ArrayList<>();
//        String sql = "SELECT * FROM account_he194197 ORDER BY total_spent " + (ascending ? "ASC" : "DESC");
//        try {
//            PreparedStatement ps = con.prepareStatement(sql);
//            ResultSet rs = ps.executeQuery();
//            while (rs.next()) {
...
//            }
//        } catch (SQLException e) {
//            System.err.println("Error at getAccountsSortedBySpent: " + e.getMessage());
//        }
//        return list;
//    }
    
    
    public void getAccountsSortedBySpending(boolean ascending) {
        accList = new Vector<>();
        String sql = "SELECT * FROM account_he194197 ORDER BY total_spent " + (ascending ? "ASC" : "DESC");
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ...
            status = "Error at getAccountsSortedBySpending: " + e.getMessage();
        }
    }

    public boolean updateTotalSpent(int accountId, float newSpent) {
        String sql = "UPDATE account_he194197 SET total_spent = ? WHERE id = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setFloat(1, newSpent);
            ps.setInt(2, accountId);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            System.err.println("Error at updateTotalSpent: " + e.getMessage());
            return false;
        }
    }
